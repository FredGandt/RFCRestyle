...
const ALL_PS = Array.from( document.querySelectorAll( "p" ) ),
	linkify = () => {
		ALL_PS.filter( p => {
			let pos = p.offsetTop - document.body.scrollTop;
			...
			p.innerHTML = p.textContent.replace( /(?:^| (RFC|BCP))(0*([0-9]+))/g, ( mtch, grp1, grp2, grp3 ) => {
				return ( grp1 ? " " : "" ) + '<a href="https://tools.ietf.org/html/' +
					( grp1 ? grp1.toLowerCase() : ( /bcp-index/.test( location.pathname ) ? "bcp" : "rfc" ) )
